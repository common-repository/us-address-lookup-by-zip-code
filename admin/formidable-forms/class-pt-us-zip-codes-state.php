 ...
 */
class Pt_Us_Zip_Codes_Public_FF_State {
	...
	public function add_field($fields){
		$fields['usz_state'] = __('US State', 'usz');
		return $fields;
    }
	...
	public function field_defaults($field_data){
		if ( $field_data['type'] == 'usz_state' ) {
			$field_data['name'] = 'State';
	...
	public function show( $field, $field_name, $atts ) {
      ...
      $field['value'] = stripslashes_deep($field['value']);
    	?>
<input type="text" id="usz-state-field" name="<?php echo esc_attr( $field_name ) ?>"
    value="<?php echo esc_attr($field['value']) ?>" readonly />
<?php
	}
}
new Pt_Us_Zip_Codes_Public_FF_State;